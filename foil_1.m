...
% Prompt user to select the parameter file via dialog box
[parfile, parpath] = uigetfile('Parfiles/*.txt', 'Select the parameter file');
if isequal(parfile,0)
    disp('User canceled parameter file selection.');
    return
end

% Extract case reference from file name (remove extension)
[~, caseref, ~] = fileparts(parfile);

parfile = fullfile(parpath, parfile);
...
for nalpha = 1:length(alpha)
    ...
    if iunt~=0
        is = ipstag + 1 - iunt;
        upperbl = sprintf('%s\n%s%5.3f', upperbl, '    Natural transition at x = ', xs(is));
    end
    if iuls~=0
        is = ipstag + 1 - iuls;
        upperbl = sprintf('%s\n%s%5.3f', upperbl, '    Laminar separation at x = ', xs(is));
        if iutr~=0
            is = ipstag + 1 - iutr;
            upperbl = sprintf('%s\n%s%5.3f', upperbl, '    Turbulent reattachment at x = ', xs(is));
        end
    end
    if iuts~=0
        is = ipstag + 1 - iuts;
        upperbl = sprintf('%s\n%s%5.3f', upperbl, '    Turbulent separation at x = ', xs(is));
    end
    upperbl = sprintf('%s\n', upperbl);
    disp(upperbl)
    ...
    if ilnt~=0
        is = ipstag + ilnt;
        lowerbl = sprintf('%s\n%s%5.3f', lowerbl, '    Natural transition at x = ', xs(is));
    end
    if ills~=0
        is = ipstag + ills;
        lowerbl = sprintf('%s\n%s%5.3f', lowerbl, '    Laminar separation at x = ', xs(is));
        if iltr~=0
            is = ipstag + iltr;
            lowerbl = sprintf('%s\n%s%5.3f', lowerbl, '    Turbulent reattachment at x = ', xs(is));
        end
    end
    if ilts~=0
        is = ipstag + ilts;
        lowerbl = sprintf('%s\n%s%5.3f', lowerbl, '    Turbulent separation at x = ', xs(is));
    end
    lowerbl = sprintf('%s\n', lowerbl);
    disp(lowerbl)
